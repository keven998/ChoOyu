...
- (void)addBlurView {
    ...
    if ((UIInterfaceOrientation *)UIDeviceOrientationIsPortrait((UIDeviceOrientation)[UIApplication sharedApplication].statusBarOrientation)) {
...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent atHeight:(float)height animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        ...
        // setup
        if (flag) { // animate
            CGRect initialFrame = CGRectMake(finalFrame.origin.x, -finalFrame.size.height, finalFrame.size.width, finalFrame.size.height);
            ...
            [UIView animateWithDuration:ANIMATION_TIME delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                blurView.alpha = self.useBlurForPopup ? 1.0f : 0.6f;
...
//            self.popupViewController.view.frame = CGRectMake(initialFrame.origin.x, [UIScreen mainScreen].bounds.size.height + initialFrame.size.height/2, initialFrame.size.width, initialFrame.size.height);
            self.popupViewController.view.frame = CGRectMake(initialFrame.origin.x,  -initialFrame.size.height, initialFrame.size.width, initialFrame.size.height);
...
- (CGRect)getPopupFrameForViewController:(UIViewController *)viewController atHeight:(float)height
{
    ...
    return CGRectMake(x, y+height, frame.size.width, frame.size.height);
}

- (void)screenOrientationChanged {
    UIView *blurView = objc_getAssociatedObject(self, &CWBlurViewKey);
    [UIView animateWithDuration:ANIMATION_TIME animations:^{
        self.popupViewController.view.frame = [self getPopupFrameForViewController:self.popupViewController atHeight:0];
        if (UIDeviceOrientationIsPortrait((UIDeviceOrientation)[UIApplication sharedApplication].statusBarOrientation)) {
