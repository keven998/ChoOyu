...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIView *centerView = [[UIView alloc] initWithFrame:CGRectMake(0, frame.size.height/2, 200, 1)];
...
-(void)createButton{
    ...
    for (int i=0; i<_arrButton.count; i++) {
        ...
        menuButton.transform = CGAffineTransformMakeRotation(-M_PI*0.5);
    ...
    MenuButton *firstBtn = [_arrButton objectAtIndex:0];
...
- (void)handleTap:(id)sender {
    ...
    pointDrag = [gesture locationInView:self];

    [UIView animateWithDuration:3 animations:^{
        [self dragPoint:pointDrag movePoint:CGPointMake(115, 200) centerPoint:center];
        
        [self reviseCirclePointWithAnimation:NO];
...
- (void)handleSinglePan:(id)sender{
    ...
    switch (panGesture.state) {
        ...
        case UIGestureRecognizerStateEnded:
        {
            CGPoint pointMove = [panGesture locationInView:self];
            [self dragPoint:pointDrag movePoint:pointMove centerPoint:center];
            [self reviseCirclePointWithAnimation:YES];
        }
            break;
...
//旋转结束后滑动到指定位置
- (void)reviseCirclePointWithAnimation: (BOOL)animation
{
    ...
    for (int i=0; i<_arrButton.count; i++) {
        ...
        if (animation) {
            ...
            [self animateWithDuration:0  animateDelay:0.0f changeIndex:i toIndex:iDest circleArray:_arrButton clockwise:isClockwise];
...
{    
    ...
    for (UIButton *oldBtn in _arrButton) {
        oldBtn.selected = NO;
